<?php
  ...
  if(isset($_POST['amount']))
  {
    ...
    {
      $all_good = false;
      $_SESSION['expense_error'] = "Variable must be a number";
    }
    ...
    {
      $all_good = false;
      $_SESSION['expense_error'] = "Variable must be less then 99999999";
    }
    ...
    {
      $all_good = false;
      $_SESSION['expense_date_error'] = "Enter expense date";
    }
    ...
    {
      $all_good = false;
      $_SESSION['expense_date_error'] = "Date must be current or earlier";
    }

    //Categorized income
    if ($isset($_POST['category_expense']))
    {
      $category = $_POST['category_expense'];
      $_SESSION['form_category_expense'] = $category;
    }
    else
    {
      $all_good = false;
      $_SESSION['expense_category_error'] = "Category must be selected";
    }
    ...
    {
      $all_good = false;
      $_SESSION['expense_comment_error'] = "Comment of expense might be 50 characters maximum.";
    }

    //validation payment
    if(isset($_POST['expense_payment'])) {
			
			$payment = $_POST['expense_payment'];
			$_SESSION['form_expense_payment'] = $payment;

		}else{
			$OK = false;
			$_SESSION['expense_error_payment'] = "Choose payment method";
		}

    //Remember entered values
    $_SESSION['form_amount_expense'] = $amount;
		$_SESSION['form_date_expense'] = $date;
		$_SESSION['form_comment_expense'] = $comment;
    ...
    try
      {
        ...
        else 
        {
          ...
          if($all_good == true)
          {
            $query_expense = "INSERT INTO expenses VALUES (NULL, '$user_id',(SELECT id FROM users_category_expenses_added WHERE user_id ='$user_id' AND name='$category'),'$amount','$date','$comment')";
            if($connection -> query($query_expense))
            {
                ...
                <form method = "post">
                    ...
                    <div class="form-floating my-2">
                        ...
                        try
                        {
                          ...
                          else
                          {
                            ...
                            $result_query_expense = $connection -> query("SELECT name FROM users_category_expenses_added WHERE user_id ='$user_id'");
                            
                            if(!$result_query_expense) throw new Exception($connection -> error);
                              
                            $how_many_names = $result_query_expense -> num_rows;
                            
                            
                            if($how_many_names > 0)
                            {
                              while ($row = $result_query_expense -> fetch_assoc())
                              {
                                echo '<div class="row">';
                                echo '<div class="col-sm-4 col-sm-offset-1">';
                                echo '<label class="radio-inline">';
                                echo '<input type="radio" name="category_expense" value="'.$row['name'];
                                
                                if(isset($_SESSION['form_expense_category']))
                                {
                                  if($row['name'] == $_SESSION['form_expense_category']) 
                                  {
                              ...
                              $result_query_expense -> free_result();
                            }
                            else
                            {
                      ...
                      ?>		
                      <?php
                        if (isset($_SESSION['error_category_expense']))
                        {
                          echo '<div class="error">'.$_SESSION['error_category_expense'].'</div>';
                          unset($_SESSION['error_category_expense']);
                        }
                      ?>
                    ...
                    <div class="form-floating my-2">
                        <input id="datepicker" width="auto" class="form-control py-3 border text-center" placeholder="" value="<?php
                          if (isset($_SESSION['form_expense_date'])){
                            echo $_SESSION['form_expense_date'];
                            unset($_SESSION['form_expense_date']);
                          }
                          ?>
                          <?php
                          if (isset($_SESSION['error_expense_date'])){
                            echo '<div class="error">'.$_SESSION['error_expense_date'].'</div>';
                            unset($_SESSION['error_expense_date']);	
                          }
                          ?>">  
                    ...
                    <?php
											...
											try{
												...
												if ($connection -> connect_errno != 0){
													...
													$result_query_expense = $connection -> query("SELECT name FROM users_payment_expenses_added WHERE user_id ='$user_id'");
													if(!$result_query_expense) throw new Exception($connection -> error);
